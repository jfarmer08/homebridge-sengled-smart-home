...
const {ElementHomeClient, Brightness} = require('./sengled-api/index');
const {RgbColor, ColorContextData, Color, ColorModeColorTemperature, ColorModeRgb} = require('./sengled-api/color');
...
SengledHubPlatform.prototype.removeAccessory = function(accessory, accessoryId = undefined) {
	if (accessory) {
		...
		try {
			this.api.unregisterPlatformAccessories("homebridge-sengled-smart-home", "Sengled", [accessory]);
		} catch (error) {
